...
IMAGE_SIZE = (64, 64)  # Размер входного изображения для субсрата
SUBSTRATE_INPUT_NODES = IMAGE_SIZE[0] * IMAGE_SIZE[1] # Количество входов субстрата
SUBSTRATE_OUTPUT_NODES = 10

# Конфигурация скрытого слоя субстрата (влияет на емкость сети)
SUBSTRATE_HIDDEN_LAYER_SIZE = (10, 10) # Размер скрытого слоя
SUBSTRATE_HIDDEN_NODES = SUBSTRATE_HIDDEN_LAYER_SIZE[0] * SUBSTRATE_HIDDEN_LAYER_SIZE[1]
SUBSTRATE_OUTPUT_NODES = 10
...
OUTPUT_DIR = 'hyperneat2.0_output_mnist'
...
def get_substrate_coords():
    ...
    for r in range(IMAGE_SIZE[0]):
        y_node = (r / (IMAGE_SIZE[0] - 1)) * 1.0 - 0.5 if IMAGE_SIZE[0] > 1 else 0.0
        for c in range(IMAGE_SIZE[1]):
            x_node = (c / (IMAGE_SIZE[1] - 1)) * 1.0 - 0.5 if IMAGE_SIZE[1] > 1 else 0.0
            input_coords.append((x_node, y_node + y_input))
    ...
    for r in range(SUBSTRATE_HIDDEN_LAYER_SIZE[0]):
        ...
        for c in range(SUBSTRATE_HIDDEN_LAYER_SIZE[1]):
            ...
            hidden_coords.append((x_node, y_node + y_hidden))
    ...
    for i in range(SUBSTRATE_OUTPUT_NODES):
        ...
        output_coords.append((x_node, y_output))
...
def generate_layer_weights(cppn, coords_layer1, coords_layer2):
...
def activate_substrate(weights_ih, weights_ho, inputs_to_substrate):
    ...
    final_outputs = np.tanh(final_inputs)
...
def eval_single_genome_hyperneat(genome, config):
    ...
    try:
        ...
        substrate_outputs_batch = activate_substrate(weights_ih, weights_ho, image_batch)
...
def run_hyperneat_parallel(config_file):
    ...
    pe = ParallelEvaluator(num_workers, eval_single_genome_hyperneat)

    gen_counter = {'gen': 0}
    def fitness_wrapper(genomes_tuples, config_inner):
        gen = gen_counter['gen']
        pe.evaluate(genomes_tuples, config_inner) # Обновление genome.fitness для каждого генома

        fits = [g.fitness for _, g in genomes_tuples]
        if not fits:
    ...
    winner_cppn_genome = p.run(fitness_wrapper, NUM_GENERATIONS)
    csv_file.close()
    with open(os.path.join(OUTPUT_DIR, 'winner_cppn_genome.pkl'), 'wb') as f:
...
def post_training_analysis(cppn_genome, config, data_holder):
    ...
    try:
        ...
        test_images_batch = data_holder.test_images
        test_labels = data_holder.test_labels

        raw_outputs_batch = activate_substrate(weights_ih, weights_ho, test_images_batch)
        preds = np.argmax(raw_outputs_batch, axis=1)
        print(f"Предсказания получены для {len(preds)} тестовых образцов.")

        raw_outputs_np = raw_outputs_batch
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(OUTPUT_DIR, 'winner_cppn_genome.pkl')
    config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         neat.DefaultSpeciesSet, neat.DefaultStagnation,
                         CONFIG_PATH)
    ...
    if os.path.exists(model_path):
        ...
        winner_cppn_genome, _, stats = run_hyperneat_parallel(CONFIG_PATH)
    ...
    # drawer = DigitDrawerHyperNEAT(winner_cppn_genome, config)
    # drawer.run()